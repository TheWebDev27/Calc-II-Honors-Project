...
The interval $(L-epsilon, L+epsilon)$ describes the range of outputs that lie within a distance $epsilon$ from $L$. One detail to note in the visualization above (Figure 2) is the presence of $delta_1$ and $delta_2$. This is because the $x$ corresponding to $L - epsilon$ does not necessarily lie the same distance away from $c$ as the $x$ corresponding to $L + epsilon$, since the rate at which $f(x)$ changes may vary as $x$ sweeps from $c - delta_1$ to $c + delta_2$. To illustrate this, notice how the curvature in the graph is steeper on the left-hand side of $c$ compared to that on the right-hand side. This means that sweeping through some range of outputs on the left requires a smaller increment of $x$ as opposed to sweeping through that same range of outputs on the right, since the output of the function increases at a faster rate on the left. Therefore, $delta_1 < delta_2$ for this particular graph.
