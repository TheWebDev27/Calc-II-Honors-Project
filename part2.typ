...
#figure(
  image("images/parabola slope.png", width: 55%),
...
The skeptical reader may question how exactly this system has solved the concern regarding rigor. In particular, how do the ideas presented so far formalize the concept of deeming a quantity in a given calculation as neglible? What has happened is that we have #emph[defined] what it means for a number to be considered negligibly small, so if a given quantity does not meet the criteria specified, then we know that it cannot be ignored. Beyond that, however, a more intimate understanding of the formalization of infinitesimals requires a working knowledge of abstract algebra. The details are obviously left out, as they lie far beyond my expertise. While the curious reader may feel discouraged or unfulfilled by this, keep in mind that the formalization of many concepts require complicated, abstract machinery, so it is commonplace for students of mathematics to take a result at face value at first and uncover its logical validity far later. As a direct analogue to the current situation, consider the analysis behind limits covered in the first half of this paper. It is likely that most calculus students who have studied limits will never encounter this rigorous treatment of them -- let alone give it any consideration. Ambitious students, however, will inevitably run across it in a real analysis course further down the road, and the formalities are unveiled then. We now lie in the very same boat with infinitesimals!
...
It is due time for us to see how infinitesimal arithemetic can be used to provide basic intuitive proofs of various ideas seen in calculus. Note that these are by no means formal proofs.
...
The extreme value theorem asserts that if a function smoothly travels over an interval of inputs, then it must take on a maximum and a minimum value at least two $x$'s.

*Continuity* entails that the outputs of a function are close to one another if the inputs are close. Another way of expressing this idea is that a small change in the input should only ever produce a proportionately small change in the output. Mathematically speaking, a function $f(x)$ is continuous on a closed interval $[a,b]$ only when
...
where the $approx$ symbol means that two quantities are _approximately_ equal.
...
where $H$ is the number of parts the interval is partitioned into. Amongst the partition points, $f(x)$ 
#v(0pt) achieves a maximum value at $x_M=display(a + (k(b-a))/H)$, so

#set align(center)

$f(x_m) >= f(x_1)$ for any $x_1=display(a + (j(b-a))/H)$

#set align(left)

#v(2pt) where $j != k$. We know that any input $x$ lies within the interval $[a, b]$ is within a distance of $display(1/(2H))$ from 
#v(0pt)one of the partition points. To see why, consider the closed interval [0,10] divided into 5 equal subintervals ($H = 5$), so the partition points are

#set align(center)

$display(0\, 0 + (10-0)/5\,0 + (2(10-0))/5\, 0 + (3(10-0))/5\,0 + (4(10-0))/5\,0 + (5(10-0))/5)$